...
export const parseFileExts = ['.js', '.ts', '.mjs', '.cjs', '.json', '.jsx', '.tsx', '.txt']
...
export async function loadConfigFile(configPath: string): Promise<ConfigFile | undefined> {
	...
	switch (ext) {
		...
		case '.ts': {
			...
			const tempConfigFilePathBase = path.resolve(dir, `${name}_temp_${generateId()}`)
...
export function generateTemplatesChoices(templateInfosArr: TemplateInfos[]) {
	...
	templateInfosArr.forEach((templateInfos, index) => {
		result[index] = templateInfos.length
			? templateInfos.map((infoItem) => {
		...
		if (filePathStat.isDirectory()) {
			...
			await promiseByStep(
				fileList.map((fileItem) => {
					return () =>
						copyToTarget(path.resolve(filePath, fileItem), targetPath, {
							overwrite
						})
				}),
				(result) => {
...
export async function requestNpmPackage(
	npmName: string,
	options?: {
		version?: string
		installDir?: string
...
export async function getLatestVersion(npmName: string) {
	return axios
		.get(`https://registry.npmjs.org/${npmName}`)
		.then((res) => {
...
export async function copyAndParseTemplate(
	filePath: string,
	targetPath: string,
	options?: {
		inside?: boolean
